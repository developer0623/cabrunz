...
package com.cabrunzltd.user.maputils;
...
public class PolyUtil {
	...
	public static boolean containsLocation(LatLng point, List<LatLng> polygon,
		...
		for (LatLng point2 : polygon) {
			...
			if (intersects(lat1, lat2, MathUtil.wrap(lng2 - lng1, -PI, PI), lat3, dLng3,
					geodesic)) {
				++nIntersect;
			}
			lat1 = lat2;
			...
			for (LatLng point2 : poly) {
				...
				if (max(lat1, lat2) >= minAcceptable
					...
					for (double x3 : xTry) {
						...
						double t = len2 <= 0 ? 0 : MathUtil.clamp((x3 * x2 + (y3 - y1)
								* dy)
								/ len2, 0, 1);
						double xClosest = t * x2;
						double yClosest = y1 + t * dy;
						double latClosest = MathUtil.inverseMercator(yClosest);
						double havDist = MathUtil.havDistance(lat3, latClosest, x3
								- xClosest);
